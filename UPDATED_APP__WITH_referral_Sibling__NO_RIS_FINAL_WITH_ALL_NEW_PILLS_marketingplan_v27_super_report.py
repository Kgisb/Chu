...
def _render_marketing_super_report(df):
    ...
    # ----- Mandatory: only rows with Calibration Slot (Deal) non-blank -----
    if cal_slot_col:
        filtered = filtered[filtered[cal_slot_col].astype(str).str.strip() != ""]
        st.caption("Filtered to rows with Calibration Slot (Deal) value present.")
    # If cal_slot_col not found, no restriction is applied.
