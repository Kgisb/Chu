...
def _render_marketing_super_report(df):
    ...
    # ---------- Global view mode: A1/B1/C1 vs JetLearn Source & Country ----------
    view_mode = st.radio(
        "View mode",
        ["A1/B1/C1 basis", "JetLearn Deal Source & Country basis"],
        index=1,
        horizontal=True,
        help="Choose whether to slice by A1/B1/C1 segment or by JetLearn Deal Source and Country filters."
    )

    filtered = base.copy()

    if view_mode == "JetLearn Deal Source & Country basis":
        col_src, col_ctry = st.columns(2)

        # JetLearn Deal Source filter
        if src_col:
            src_vals = sorted([str(x) for x in filtered[src_col].dropna().unique()])
            with col_src:
                sel_src = st.multiselect(
                    "JetLearn Deal Source",
                    ["All"] + src_vals,
                    default=["All"],
                    help="Filter deals by JetLearn Deal Source."
                )
            if sel_src and "All" not in sel_src:
                filtered = filtered[filtered[src_col].astype(str).isin(sel_src)]

        # Country filter
        if ctry_col:
            ctry_vals = sorted([str(x) for x in filtered[ctry_col].dropna().unique()])
            with col_ctry:
                sel_ctry = st.multiselect(
                    "Country",
                    ["All"] + ctry_vals,
                    default=["All"],
                    help="Filter deals by Country."
                )
            if sel_ctry and "All" not in sel_ctry:
                filtered = filtered[filtered[ctry_col].astype(str).isin(sel_ctry)]
        ...
        if seg_choice != "All":
            if seg_col:
                ...
                st.warning("A1/B1/C1 column not found; showing all data.")
    ...
    def _run_window(label: str, key_prefix: str, work_base: pd.DataFrame):
        ...
        if not sel_metrics and not sel_add_cols:
            st.warning("Select at least one metric أو additional column.")
            return None
        ...
        for g, sub in work.groupby("_grp"):
            ...
            # Base MTD on Create Date; Cohort uses event dates explicitly
    ...
    # Combined comparison if both windows have data
