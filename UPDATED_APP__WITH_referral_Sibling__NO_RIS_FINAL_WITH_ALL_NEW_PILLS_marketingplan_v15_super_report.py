...
def _render_marketing_super_report(df):
    ...
    # --- Booking type flag ---
    # Pre-book = Calibration Slot (Deal) has value
    # Sales-book = Slot blank AND (Cal Resched Date else First Cal Date) present
    if cal_slot_col:
    ...
    # For sales-book, pick Resched if present, else First Cal
    sales_event = rcal_dt.copy()
    sales_event = sales_event.where(pd.notna(sales_event), fcal_dt)

    is_sales = (~is_pre) & pd.notna(sales_event)
    is_unknown = (~is_pre) & (~pd.notna(sales_event))

    work["_booking_type"] = np.where(
        is_pre, "Pre-book",
        np.where(is_sales, "Sales-book", "Unknown")
    )
    ...
    with c2:
        bt_opts = ["Pre-book", "Sales-book", "Unknown"]
        sel_bt = st.multiselect(
            "Booking type",
            bt_opts,
            default=["Pre-book", "Sales-book"],
            help="Pre-book: Slot filled. Sales-book: Slot blank & Cal Resched/First Cal present. Unknown: others."
        )
    ...
    for g, sub in work.groupby("_grp"):
        ...
        # Pre-book and Sales-book counts within selected window
        if mode == "MTD" and create_col:
            pb_mask = base_mtd & (sub["_booking_type"] == "Pre-book")
            sb_mask = base_mtd & (sub["_booking_type"] == "Sales-book")
        else:
            cd_in_range = (sub["_cd"] >= start) & (sub["_cd"] <= end)
            pb_mask = cd_in_range & (sub["_booking_type"] == "Pre-book")
            sb_mask = cd_in_range & (sub["_booking_type"] == "Sales-book")
        rec["Pre-book count"] = int(pb_mask.sum())
        rec["Sales-book count"] = int(sb_mask.sum())
    ...
    # Keep selected metrics only
    keep_cols = ["Group"]
    for m in metric_options:
        if m in sel_metrics and m in agg.columns:
            keep_cols.append(m)

    # Always show booking type split counts if available
    if "Pre-book count" in agg.columns:
        keep_cols.append("Pre-book count")
    if "Sales-book count" in agg.columns:
        keep_cols.append("Sales-book count")
