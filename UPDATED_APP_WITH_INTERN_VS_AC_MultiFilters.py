...
def _render_marketing_intern_vs_ac(df):
    ...
    Counts are based ONLY on event dates:
    ...
    Two windows (A & B), each with:
      - Today / Yesterday / This Month / Last Month / Custom

    Filters:
      - Segment type: Intern / AC (multi-select)
      - Segment names:
          * For Intern: multi-select specific intern names or All
          * For AC: multi-select specific AC names or All
      - Breakdown:
          * Overall
          * Country (multi-select; All or list)
          * JetLearn Deal Source (multi-select; All or list)
    ...
    # 3) Breakdown with multi-select values
    breakdown = st.radio(
        "Breakdown by",
    ...
    if dim_col is not None:
        ...
        if (not dim_values) or ("All" in dim_values):
            dim_values = None
            dim_label = f"{breakdown}: All"
        else:
            dim_label = f"{breakdown}: {', '.join(dim_values)}"
    ...
    def _metrics_for(seg_name: str, start, end):
        ...
        # breakdown filters
        if dim_col is not None and dim_values:
            base_mask &= df[dim_col].astype(str).isin(dim_values)
