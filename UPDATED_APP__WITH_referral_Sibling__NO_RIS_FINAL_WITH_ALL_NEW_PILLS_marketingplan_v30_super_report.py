...
def _render_marketing_super_report(df):
    ...
    if cal_slot_col:
        only_with_slot = st.checkbox(
            ...
            value=False,
            help="When enabled, both Window A and Window B include only deals where Calibration Slot (Deal) has a real value (not blank / not NaN)."
        )
        if only_with_slot:
            _slot_series = filtered[cal_slot_col]
            mask = _slot_series.notna() & (_slot_series.astype(str).str.strip() != "") & (_slot_series.astype(str).str.lower() != "nan")
            filtered = filtered[mask]
    # If cal_slot_col not found, this toggle is not shown.
    ...
    # If deal_name_col not found, this control is hidden.
