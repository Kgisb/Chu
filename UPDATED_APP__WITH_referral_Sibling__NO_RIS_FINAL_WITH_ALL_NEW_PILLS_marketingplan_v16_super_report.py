...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Trend", "Deal Score Threshold", "Invalid Deals", "Super report",
        "Marketing Plan"],
    }
...
def _render_marketing_super_report(df):
    ...
    st.subheader("Super report — Threshold × Booking type × Key events")
    ...
    cal_book_col = _find_col(work, [
        "Calibration Booking Date",
        "Calibration Booked Date",
        "Calibration booking date",
    ])

    ac_col = _find_col(work, [
    ...
    # Pre-book:
    #   Calibration Slot (Deal) has some value.
    # Self-book:
    #   Calibration Slot (Deal) is blank
    #   AND use Calibration Rescheduled Date if present,
    #       else First Calibration Scheduled Date if present.
    # Unknown:
    #   All remaining cases.

    if cal_slot_col:
        slot_ser = work[cal_slot_col].astype(str).str.strip()
        is_pre = slot_ser != ""
    else:
        # If no slot column, nothing can be pre-book by definition
        is_pre = pd.Series(False, index=work.index)

    def _to_dt(s):
        return pd.to_datetime(s, errors="coerce", dayfirst=True)

    fcal_dt = _to_dt(work[first_cal_col]) if first_cal_col else pd.NaT
    rcal_dt = _to_dt(work[cal_resch_col]) if cal_resch_col else pd.NaT

    # For self-book, prefer reschedule, else first calibration
    self_event = rcal_dt.copy()
    self_event = self_event.where(self_event.notna(), fcal_dt)

    is_self = (~is_pre) & self_event.notna()
    is_unknown = (~is_pre) & (~is_self)

    work["_booking_type"] = np.where(
        is_pre, "Pre-book",
        np.where(is_self, "Self-book", "Unknown")
    )
    work["_self_event"] = self_event

    # Core date fields for metrics
    work["_cd"] = _to_dt(work[create_col]) if create_col else pd.NaT
    work["_payd"] = _to_dt(work[pay_col]) if pay_col else pd.NaT
    work["_fcd"] = fcal_dt
    work["_rcd"] = rcal_dt

    if cal_book_col:
        work["_bkd"] = _to_dt(work[cal_book_col])
    elif cal_slot_col:
        # if slot is a datetime-like string, use it; else it will become NaT
        work["_bkd"] = _to_dt(work[cal_slot_col])
    else:
        work["_bkd"] = pd.NaT

    # ========== Controls row 1: Threshold, Booking, Mode ==========
    c1, c2, c3 = st.columns([1.7, 1.5, 1.2])
    ...
    with c2:
        bt_opts = ["Pre-book", "Self-book", "Unknown"]
        sel_bt = st.multiselect(
            "Booking type",
            bt_opts,
            default=["Pre-book", "Self-book"],
            help="Pre-book: Slot filled. Self-book: Slot blank & Cal Resched/First Cal present."
        )
    ...
    # ========== Controls row 2: Group, Metrics, Chart ==========
    c4, c5, c6 = st.columns([1.9, 1.6, 1.3])
    ...
    if not sel_metrics:
        st.warning("Select at least one metric to view Super report.")
        return
    ...
    if preset == "Today":
        ...
        c7, c8 = st.columns(2)
        with c7:
            start = st.date_input("Start date", value=today - pd.Timedelta(days=29), key="sr_start")
        with c8:
            end = st.date_input("End date", value=today, key="sr_end")
    ...
    if sel_bt is not None and len(sel_bt) > 0:
        work = work[work["_booking_type"].isin(sel_bt)]
    ...
    for g, sub in work.groupby("_grp"):
        ...
        # Base MTD window on Create Date; Cohort windows per-event below
        ...
        # First calibration date count
        if "First calibration date count" in sel_metrics:
            if mode == "MTD":
        ...
        # Calibration reschedule count
        if "Calibration reschedule count" in sel_metrics:
            if mode == "MTD":
        ...
        # Always compute Pre-book and Self-book counts, aligned to date window on Create Date
        if mode == "MTD" and create_col:
            pb_mask = base_mtd & (sub["_booking_type"] == "Pre-book")
            sb_mask = base_mtd & (sub["_booking_type"] == "Self-book")
        else:
            cd_in_range = (sub["_cd"] >= start) & (sub["_cd"] <= end)
            pb_mask = cd_in_range & (sub["_booking_type"] == "Pre-book")
            sb_mask = cd_in_range & (sub["_booking_type"] == "Self-book")

        rec["Pre-book count"] = int(pb_mask.sum())
        rec["Self-book count"] = int(sb_mask.sum())

        # Conversion %
        if ("Deal count" in rec and "Enrollment" in rec and
                rec.get("Deal count", 0) > 0):
            rec["Enrollment / Deal %"] = round(rec["Enrollment"] * 100.0 / rec["Deal count"], 2)
    ...
    # Keep selected metrics only + always show booking type split & conversion %
    keep_cols = ["Group"]
    for m in metric_options:
        if m in sel_metrics and m in agg.columns:
            keep_cols.append(m)

    if "Pre-book count" in agg.columns:
        keep_cols.append("Pre-book count")
    if "Self-book count" in agg.columns:
        keep_cols.append("Self-book count")

    if "Enrollment / Deal %" in agg.columns:
        keep_cols.append("Enrollment / Deal %")

    agg = agg[keep_cols]

    # ========== Table ==========
    st.markdown("#### Super report table")
    st.dataframe(
    ...
    st.markdown("#### Super report chart")
