...
def _render_marketing_intern_vs_ac(df):
    """
    Intern vs AC comparison based ONLY on event dates.
    ...
    Counts are computed on each event's own date.
    Example: if you select Oct 2025–Nov 2025, we count how many
    First Cal / Resched / Done / Trigger / Enrol events fall in that range.

    Features:
      - Window A and Window B
      - Filter by Country or JetLearn Deal Source (or Overall)
      - Segment filter: Intern, AC, or both
      - Conversion:
          % Cal Done / First Cal
          % Enrolments / Cal Done
    """
    ...
    def _select_window(label: str, key_prefix: str):
        ...
        with c2:
            st.markdown("Event-date based", help="Counts use event dates only.")
    ...
    # metrics from event dates
    def _metrics_for(seg_name: str, start, end):
    ...
    for label, (ws, we) in {
        ...
        for seg_name in selected_segs:
            met = _metrics_for(seg_name, ws, we)
            rows.append({
                "Window": label,
                "Segment": seg_name,
    ...
    st.download_button(
        "Download CSV — Intern vs AC (event-date based)",
        data=summary_df.to_csv(index=False).encode("utf-8"),
        file_name="intern_vs_ac_event_date_based.csv",
        mime="text/csv",
        key="iva_dl"
    )
