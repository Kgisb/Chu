...
def _render_marketing_super_report(df):
    ...
    st.subheader("Super report — Threshold × Key events")
    ...
    # ========== Controls row 2: Group, Metrics, Add Columns, Chart ==========
    c4, c5, c6 = st.columns([1.7, 1.7, 1.4])
    ...
    with c6:
        sel_add_cols = st.multiselect(
            ...
            add_cols_options,
            default=[],
            help="Always include these columns when selected."
        )
    ...
    if not sel_metrics and not sel_add_cols:
        st.warning("Select at least one metric or additional column to view Super report.")
        return
    ...
    for g, sub in work.groupby("_grp"):
        ...
        # First calibration date count (always computable for add-cols)
        ...
        # Calibration reschedule count (always computable for add-cols)
        ...
        # Calibration done count (based on calibration booking date)
        if "Calibration done count" in sel_add_cols:
            if mode == "MTD":
                rec["Calibration done count"] = int((base_mtd & sub["_bkd"].notna()).sum())
            else:
                rec["Calibration done count"] = int(((sub["_bkd"] >= start) & (sub["_bkd"] <= end)).sum())
    ...
    for m in sel_add_cols:
        if m in agg.columns and m not in keep_cols:
            keep_cols.append(m)

    if "Enrollment / Deal %" in agg.columns and "Enrollment / Deal %" not in keep_cols:
        keep_cols.append("Enrollment / Deal %")

    # Fallback if user somehow left everything blank
    if len(keep_cols) == 1 and len(agg.columns) > 1:
        keep_cols += [c for c in agg.columns if c != "Group"][:3]
