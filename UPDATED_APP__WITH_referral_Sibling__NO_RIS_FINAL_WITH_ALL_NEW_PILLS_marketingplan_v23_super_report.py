...
def _render_marketing_super_report(df):
    ...
    if view_mode == "JetLearn Deal Source & Country basis" and src_col and ctry_col:
        ...
        st.caption("Define one or more (JetLearn Deal Source, Countries) groups. Data will be the UNION of all groups.")

        group_masks = []

        for i in range(n_rows):
            col_src, col_ctry = st.columns([1.4, 2.0])
        ...
        with col_reset:
            if st.button("Reset groups", key="sr_reset_pair"):
                for i in range(n_rows + 5):
                    st.session_state.pop(f"sr_src_{i}", None)
                    st.session_state.pop(f"sr_ctry_{i}", None)
                st.session_state["sr_pair_rows"] = 1
                group_masks = []
    ...
    def _run_window(label: str, key_prefix: str, work_base: pd.DataFrame):
        ...
        for g, sub in work.groupby("_grp"):
            ...
            # Base MTD on Create Date; Cohort considers event dates via conditions
