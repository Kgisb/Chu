...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Trend", "Deal Score Threshold", "Invalid Deals", "Super report", "Marketing Plan"],
    }
...
if st.session_state.get("nav_master") == "Performance" and st.session_state.get("nav_sub") == "Cash-in":
    _render_performance_cash_in()



# ================================
# Performance ▶ Cash-in (Google Sheet driven)
# ================================
def _render_performance_cash_in():
    """Cash-in view driven from the specified Google Sheet.

    Source:
      https://docs.google.com/spreadsheets/d/1ygmNCuERA2kkWGztQCmX9sTZlyTZcLp_UM0_5GPZxN4/edit?gid=0#gid=0

    Layout:
      - First table (Cash-in Summary): range A1:BB13 (top)
      - Second table (Cash-in Totals): range A15:D15 (below)
      - Row A14:D14 (any 'Total' row) is ignored.
    """
    import streamlit as st
    import pandas as pd

    SHEET_ID = "1ygmNCuERA2kkWGztQCmX9sTZlyTZcLp_UM0_5GPZxN4"
    BASE_EXPORT = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/export?format=csv"

    # Summary block: A1:BB13
    url_summary = BASE_EXPORT + "&gid=0&range=A1:BB13"
    # Totals block: A15:D15
    url_totals = BASE_EXPORT + "&gid=0&range=A15:D15"

    try:
        df_summary = pd.read_csv(url_summary)
    except Exception as e:
        st.error(f"Unable to load Cash-in summary (A1:BB13): {e}")
        return

    try:
        df_totals = pd.read_csv(url_totals, header=None)
        # If it's a single-row KPI row, set compact column names
        if df_totals.shape[0] == 1:
            df_totals.columns = [f"Col{i+1}" for i in range(df_totals.shape[1])]
    except Exception as e:
        st.error(f"Unable to load Cash-in totals (A15:D15): {e}")
        return

    st.subheader("Cash-in — Summary (A1:BB13)")
    st.dataframe(df_summary, use_container_width=True)

    st.subheader("Cash-in — Totals (A15:D15)")
    st.dataframe(df_totals, use_container_width=True)


# ---- Dispatcher for Performance ▸ Cash-in
if st.session_state.get("nav_master") == "Performance" and st.session_state.get("nav_sub") == "Cash-in":
    _render_performance_cash_in()
