...
def _render_marketing_intern_vs_ac(df):
    ...
    Counts use the respective event dates only.

    Filters:
      - Segment: Intern / AC (only rows where the respective name column is non-empty)
      - Breakdown: Overall / Country / JetLearn Deal Source
      - Window A and Window B (Today / Yesterday / This Month / Last Month / Custom)
    ...
    # segment labels: only rows with valid names
    ...
    # segment filter: controls which segment rows are considered
    seg_opts = ["Intern", "AC"]
    ...
    # metrics from event dates, respecting selected segments
