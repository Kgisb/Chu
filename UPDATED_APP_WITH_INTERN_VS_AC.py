...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Intern vs AC", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Trend", "Deal Score Threshold", "Invalid Deals", "Super report", "Marketing Plan"],
    }
...
def _render_marketing_intern_vs_ac(df):
    '''
    Compare Sales Intern vs Academic Counsellor funnel performance in two windows (A & B),
    with MTD (Create-date based) vs Cohort (event-date based) logic.

    Metrics:
      - First Calibration / Trial Scheduled count
      - Calibration Rescheduled count
      - Calibration Done / Trial Done count
      - [Trigger] - Calibration Booking Date count
      - Enrollments (Payment Received Date)
      - % Cal Done vs First Cal
      - % Enrolments vs Cal Done

    Breakdown options:
      - Overall
      - By Country
      - By JetLearn Deal Source
    '''
    import streamlit as st
    import pandas as pd
    from datetime import date, timedelta

    st.subheader("Marketing — Intern vs AC (Window A vs Window B)")
    ...
    # helpers (fallbacks use existing globals if available)
    try:
        _find_col = find_col
    except NameError:
    ...
    try:
        _coerce_dt = coerce_datetime
    except NameError:
        def _coerce_dt(series: pd.Series) -> pd.Series:
            return pd.to_datetime(series, errors="coerce", dayfirst=True)

    intern_col = _find_col(df, [
        "Sales - Intern", "Sales Intern", "Sales intern", "Sales_Intern",
        "Sales Intern Name", "Intern Name"
    ])
    ac_col = _find_col(df, [
        "Student/Academic Counsellor", "Student / Academic Counsellor",
        "Academic Counsellor", "AC Name", "Owner"
    ])
    create_col = _find_col(df, [
        "Create Date", "Created Date", "Deal Create Date", "Created On",
        "Creation Date", "Deal Created Date", "Create_Date"
    ])
    ...
    # segment label: Intern vs AC
    seg = pd.Series("Other", index=df.index)
    if intern_col:
        is_intern = df[intern_col].astype(str).str.strip() != ""
        seg[is_intern] = "Intern"
    else:
        is_intern = pd.Series(False, index=df.index)

    if ac_col:
        is_ac = (df[ac_col].astype(str).str.strip() != "") & ~is_intern
        seg[is_ac] = "AC"
    else:
    ...
    if not mask_seg.any():
        st.warning("No rows tagged as Sales Intern or Academic Counsellor.")
        return
    ...
    # dates
    create_dt = _coerce_dt(df[create_col]) if create_col else pd.Series(pd.NaT, index=df.index)
    ...
    breakdown = st.radio(
        "View by",
        ["Overall", "Country", "JetLearn Deal Source"],
        index=0,
        horizontal=True,
        key="iva_breakdown"
    )
    ...
    if breakdown == "Country":
        if country_col:
            ...
            st.warning("Country column not found; showing Overall only.")
            breakdown = "Overall"
    elif breakdown == "JetLearn Deal Source":
        if src_col:
            dim_col = src_col
        else:
            st.warning("JetLearn Deal Source column not found; showing Overall only.")
            breakdown = "Overall"

    dim_value = "All"
    if dim_col is not None:
        all_vals = sorted(str(v) for v in df[dim_col].dropna().unique())
        options = ["All"] + all_vals
        dim_value = st.selectbox(
            f"{breakdown} filter",
            options,
            index=0,
            key="iva_dim_value"
        )
        dim_label = f"{breakdown}: {dim_value}" if dim_value != "All" else f"{breakdown}: All"
    ...
    def _select_window(label: str, key_prefix: str):
        ...
        with c2:
            mode = st.radio(
                f"{label} mode",
                ["MTD (by Create Date)", "Cohort (by Event Dates)"],
                index=0,
                horizontal=True,
                key=f"{key_prefix}_mode"
            )
        ...
        return start_ts, end_ts, mode

    st.markdown("**Configure comparison windows**")
    wA_start, wA_end, wA_mode = _select_window("Window A", "iva_A")
    wB_start, wB_end, wB_mode = _select_window("Window B", "iva_B")

    def _metrics_for(seg_name: str, start, end, mode: str):
        base_mask = (seg == seg_name)
        if dim_col is not None and dim_value != "All":
            base_mask &= (df[dim_col].astype(str) == dim_value)
        ...
        if mode.startswith("MTD"):
            if create_col is None:
                scope_mask = base_mask
            else:
                scope_mask = base_mask & create_dt.between(start, end)
            fc_mask = scope_mask & first_cal_dt.notna() if first_cal_col else pd.Series(False, index=df.index)
            rs_mask = scope_mask & resched_dt.notna() if resched_col else pd.Series(False, index=df.index)
            dn_mask = scope_mask & done_dt.notna() if done_col else pd.Series(False, index=df.index)
            tg_mask = scope_mask & trig_dt.notna() if trig_col else pd.Series(False, index=df.index)
            py_mask = scope_mask & pay_dt.notna() if pay_col else pd.Series(False, index=df.index)
        else:
            ...
            scope_mask = fc_mask | rs_mask | dn_mask | tg_mask | py_mask

        deals_scope = int(scope_mask.sum())
    ...
    rows = []
    for label, (ws, we, wm) in {
        "Window A": (wA_start, wA_end, wA_mode),
        "Window B": (wB_start, wB_end, wB_mode),
    }.items():
        for seg_name in ["Intern", "AC"]:
            met = _metrics_for(seg_name, ws, we, wm)
            rows.append({
                "Window": label,
                "Mode": "MTD" if wm.startswith("MTD") else "Cohort",
                "Segment": seg_name,
                "Deals in scope": met["Deals in scope"],
    ...
    st.markdown(f"**Breakdown:** {dim_label}")
    st.dataframe(summary_df, use_container_width=True, hide_index=True)
    ...
    st.download_button(
        "Download CSV — Intern vs AC comparison",
        data=summary_df.to_csv(index=False).encode("utf-8"),
        file_name="intern_vs_ac_comparison.csv",
        mime="text/csv",
        key="iva_dl"
    )
...
if st.session_state.get("nav_master") == "Marketing" and st.session_state.get("nav_sub") == "Intern vs AC":
    ...
    if _df_any is not None:
        ...
        import streamlit as _st
        _st.error("No dataframe available to render Intern vs AC.")
