...
def _render_marketing_super_report(df):
    ...
    # --- Booking type flag ---
    # Pre-book = Calibration Slot (Deal) has value; Self-book = blank
    if cal_slot_col:
        slot_ser = work[cal_slot_col].astype(str).str.strip()
        work["_booking_type"] = np.where(slot_ser != "", "Pre-book", "Self-book")
    else:
        work["_booking_type"] = "Unknown"
    ...
    work["_fcd"] = _to_dt(work[first_cal_col]) if first_cal_col else pd.NaT
    work["_rcd"] = _to_dt(work[cal_resch_col]) if cal_resch_col else pd.NaT

    # If no explicit booking date, fallback to slot as proxy
    if cal_book_col:
    ...
    with c2:
        if cal_slot_col:
            bt_opts = ["Pre-book", "Self-book"]
            sel_bt = st.multiselect(
                "Booking type",
                bt_opts,
                default=bt_opts,
                help="Pre-book = Calibration Slot (Deal) filled; Self-book = slot blank."
            )
        else:
            sel_bt = None
            st.caption("No 'Calibration Slot (Deal)' column — booking type filter disabled.")
    ...
    with c3:
        mode = st.radio(
            ...
            index=0,
            horizontal=True,
            help="MTD: Filter window on Create Date. Cohort: Filter window on each event date."
        )
    ...
    def _month_bounds(d):
        return (
            ...
            pd.Timestamp(d.year, d.month, _monthrange(d.year, d.month)[1])
        )
    ...
    for g, sub in work.groupby("_grp"):
        ...
        # Base mask for MTD (on Create Date)
        ...
        # Conversion helper
        if "Deal count" in rec and "Enrollment" in rec and rec["Deal count"] > 0:
