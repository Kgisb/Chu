...
def _render_marketing_super_report(df):
    ...
    # Self-book = NOT Pre-book AND at least one of (Trial, First Cal, Cal Resch) exists
    if cal_slot_col:
    ...
    has_self_signal = ((pd.notna(fcal_dt)) | (pd.notna(rcal_dt)))

    booking_type = np.where(is_pre, "Pre-book",
                      np.where(has_self_signal, "Self-book", "Unknown"))
    work["_booking_type"] = booking_type

    # --- Core date fields for later metrics ---
    ...
    with c2:
        ...
        sel_bt = st.multiselect(
            ...
            help="Pre-book = Slot filled; Self-book = no slot but trial/cal activity; Unknown = none."
        )
    ...
    with c3:
        mode = st.radio(
            ...
            index=0,
            horizontal=True,
            help="MTD: window on Create Date. Cohort: window on each event date."
        )
    ...
    for g, sub in work.groupby("_grp"):
        ...
        if ("Deal count" in rec and "Enrollment" in rec and
                rec["Deal count"] is not None and rec["Deal count"] > 0):
